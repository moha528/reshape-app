...
import { Icon, fmt, fmtDur, relTime } from './icons.jsx';
import { MOCK } from './data.js';
import { ConnectorBadge, StatusBadge } from './dashboard.jsx';
import { initialLogs } from './execution.jsx';
...
export function History({ navigate }) {
  ...
  let rows = MOCK.MIGRATIONS.filter(m => filter === 'all' || m.status === filter);
    ...
    <div className="page-enter">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Toutes les migrations de votre organisation.</p>
        </div>
...
export function MigrationDetail({ navigate }) {
  const m = MOCK.MIGRATIONS[1];
  ...
  return (
    <div className="page-enter">
      ...
      <div className="grid grid-cols-4" style={{ marginBottom: 20 }}>
        ...
        <div className="stat-card"><div className="stat-label"><Icon name="coins" /> Crédits</div><div className="stat-value">{fmt(m.credits)}</div><div className="stat-trend flat">Estimé: {fmt(m.credits + 30)} (−3%)</div></div>
      </div>
      ...
      {tab === 'overview' && (
        <div className="grid" style={{ gridTemplateColumns: '2fr 1fr', gap: 20 }}>
          <div className="col gap-4">
            ...
            <div className="card">
              <div className="card-header"><div><h3 className="card-title">Distribution des lignes</h3></div></div>
          ...
          <div className="col gap-4">
            <div className="card">
              ...
              <div className="card-body">
                ...
                <div style={{ fontSize: 11.5, color: 'var(--warning)', marginTop: 12, padding: 10, background: 'var(--warning-bg)', borderRadius: 8, border: '1px solid var(--warning-border)' }}>
                  <Icon name="clock" size={11} /> Suppression dans <strong>27 jours</strong>
                </div>
              </div>
            </div>
          </div>
      ...
      {tab === 'errors' && (
        <div className="card">
          <div className="card-header"><div><h3 className="card-title">3 lignes en erreur</h3></div>
            <button className="btn btn-secondary btn-sm"><Icon name="download" size={12} /> errors.csv</button>
          </div>
      ...
      {tab === 'logs' && (
        <div className="card"><div className="card-body" style={{ padding: 16 }}>
          <div className="log-stream" style={{ height: 480 }}>
            ...
            <div className="log-line"><span className="log-time">{new Date().toLocaleTimeString('fr-FR')}</span><span className="log-level OK">OK</span><span className="log-msg">Migration terminée · 89 432 lignes commitées</span></div>
          </div>
        </div></div>
      )}
...
export function Templates({ navigate }) {
  return (
    <div className="page-enter">
      <div className="page-header">
        <div>
      ...
      <div className="grid grid-cols-3" style={{ gap: 16 }}>
        {MOCK.TEMPLATES.map(t => (
          <div key={t.id} className="card" style={{ padding: 18, cursor: 'pointer' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11.5, color: 'var(--text-tertiary)', borderTop: '1px solid var(--border-subtle)', paddingTop: 10 }}>
              <span>{t.owner}</span>
  ...
  return (
    <div className="page-enter">
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        <div className="card-header"><div><h3 className="card-title">Connexions BDD sauvegardées</h3><div className="card-subtitle">4 connexions · AES-256</div></div></div>
        <div className="table-wrap" style={{ borderRadius: 0, border: 'none' }}>
          <table className="table">
...
function CreditsChart() {
  const days = 30;
  const consumed = React.useMemo(() => Array.from({ length: days }, (_, i) => 200 + Math.sin(i * 0.5) * 600 + Math.random() * 400), []);
      ...
      <div className="modal" style={{ maxWidth: 720 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div style={{ marginTop: 20, padding: 14, borderRadius: 10, background: 'var(--bg-surface-2)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 10, fontSize: 12, color: 'var(--text-secondary)' }}>
            <Icon name="shield" size={14} /> Paiement TVA gérée par Paddle. CB, PayPal, virement.
          </div>
        </div>
...
export function Credits() {
  ...
  return (
    <div className="page-enter">
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        <div className="card-header"><div><h3 className="card-title">Consommation des 30 derniers jours</h3></div></div>
      ...
      {tab === 'members' && (
        <div className="card">
          <div className="table-wrap" style={{ borderRadius: 12, border: 'none' }}>
            <table className="table">
              <thead><tr><th>Membre</th><th>Email</th><th>Rôle</th><th>2FA</th><th>Migrations</th><th>Crédits</th><th>Dernière connexion</th><th></th></tr></thead>
              <tbody>
                {MOCK.USERS.map((u, i) => (
                  <tr key={i}>
                    <td>
    ...
    <div className="page-enter">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Toutes les actions significatives de votre organisation.</p>
        </div>
        <div className="page-header-actions">
        ...
        <div className="card">
          ...
          <div className="card-body">
            ...
            <div className="alert alert-info" style={{ marginTop: 14 }}>
              <Icon name="info" />
              <div className="alert-body" style={{ fontSize: 12 }}>L'API REST publique est prévue post-MVP.</div>
            </div>
          </div>
        </div>
      </div>
              ...
              <div style={{ borderTop: '1px solid var(--border)', paddingTop: 20 }}>
                ...
                {[
                  ...
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: i === 0 ? '1px solid var(--border-subtle)' : 'none' }}>
                    <div>
                      <div style={{ fontWeight: 550, fontSize: 13 }}>{s.d} {s.cur && <span className="badge badge-success" style={{ marginLeft: 6 }}>Actuelle</span>}</div>
                      <div style={{ fontSize: 11.5, color: 'var(--text-tertiary)' }} className="mono">{s.loc} · {s.ip}</div>
                    </div>
          ...
          {tab === 'preferences' && (
            <div className="card-body" style={{ padding: 24 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                {[
                  { l: 'Thème', d: 'Clair, sombre ou auto.', opts: ['Clair', 'Sombre', 'Auto'], sel: 0 },
                  { l: 'Langue de l\'interface', d: 'Français · English (early access)', opts: null },
                ].map((r, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div><div style={{ fontWeight: 550 }}>{r.l}</div><div style={{ fontSize: 12, color: 'var(--text-tertiary)' }}>{r.d}</div></div>
                    {r.opts ? (
                      <div style={{ display: 'flex', gap: 4, background: 'var(--bg-surface-2)', borderRadius: 8, padding: 3, border: '1px solid var(--border)' }}>
                        {r.opts.map((t, j) => <button key={t} style={{ padding: '5px 12px', fontSize: 12, fontWeight: 550, borderRadius: 6, background: j === r.sel ? 'var(--bg-surface)' : 'transparent', color: j === r.sel ? 'var(--text-primary)' : 'var(--text-tertiary)' }}>{t}</button>)}
                      </div>
                    ) : (
          ...
          {tab === 'notifications' && (
            <div className="card-body" style={{ padding: 24 }}>
              ...
              {[
                ...
                { l: 'Rétention de fichiers', e: true, i: false, p: false },
            ...
            <div className="card-body" style={{ padding: 24 }}>
              ...
              <div style={{ border: '1px solid var(--danger-border)', borderRadius: 10, padding: 16, marginBottom: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div><div style={{ fontWeight: 600, fontSize: 13 }}>Exporter toutes mes données</div><div style={{ fontSize: 12, color: 'var(--text-tertiary)' }}>Conformité RGPD · archive ZIP</div></div>
                  <button className="btn btn-secondary"><Icon name="download" /> Exporter</button>
                </div>
              </div>
              <div style={{ border: '1px solid var(--danger-border)', borderRadius: 10, padding: 16, background: 'var(--danger-bg)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div><div style={{ fontWeight: 600, fontSize: 13 }}>Supprimer mon compte</div><div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Suppression définitive sous 30 jours.</div></div>
