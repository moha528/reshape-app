...
import { Icon, fmt, relTime } from './icons.jsx';
...
export function ConnectorBadge({ id }) {
  const c = MOCK.CONNECTORS.find(x => x.id === id);
  if (!c) return null;
  return (
...
export function Dashboard({ navigate }) {
  ...
  const running = MOCK.MIGRATIONS.filter(m => m.status === 'running');
  const recent = MOCK.MIGRATIONS.slice(0, 5);
